...
data = data[['Fecha'] + [col for col in data.columns if col != 'Fecha']]


data['Rendimientos'] = (data['Close']-data['Open'])/data['Open'] 
# Con la formula de arriba se reduce la cantidad de valores no disponibles, la formula de 
# abajo calcula los rendimientos de forma logaritmica
#data['Rendimientos'] = data['Último'].pct_change(fill_method ='bfill')

desc_stats = data['Rendimientos'].describe()


jb_test = jarque_bera(data['Rendimientos'])

mean = data['Rendimientos'].mean()
std = data['Rendimientos'].std()
ks_test = kstest(data['Rendimientos'], 'norm', args=(mean, std))
